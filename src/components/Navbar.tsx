...
export default function Navbar() {
  ...
  return (
    <header className={`fixed top-0 left-0 w-full h-18 backdrop-blur-md border-b border-glaucous-200/80 z-50 flex items-center transition-all duration-300 translate-y-0 ${
      isScrolled ? "shadow-[0_4px_24px_rgba(36,36,35,0.06)] bg-glaucous-50/95" : "shadow-none bg-glaucous-50/75"
    }`}>
      <div className="w-full max-w-7xl mx-auto px-6 flex justify-between items-center">
        <Link
          href="/"
          className={`flex items-center origin-left transition-all duration-500 ease-out ${
            isScrolled
              ? "scale-100 translate-y-0 bg-transparent border-transparent p-0 shadow-none backdrop-blur-none"
              : "scale-[1.4] md:scale-[2.2] translate-y-5 md:translate-y-11 bg-white/20 backdrop-blur-xl border border-white/25 border-t-white/40 border-b-black/15 px-4 py-2.5 md:px-4 md:py-4 shadow-[0_20px_50px_rgba(36,36,35,0.12),inset_0_1px_2px_rgba(255,255,255,0.4)] rounded-2xl md:rounded-3xl"
          }`}
        >
          <Logo className="h-16 md:h-20 w-auto transition-all duration-500" />
        </Link>
