...
export default function IndustriesSection() {
  ...
  const renderCoachingWidget = () => {
    ...
    return (
      <div className="bg-white border border-glaucous-200 rounded-xl p-4 shadow-sm space-y-3 font-sans text-sm">
        <div className="flex items-center justify-between border-b border-glaucous-100 pb-2">
          <span className="font-mono text-xs font-bold text-glaucous-600">FEE MANAGEMENT PANEL</span>
          <span className="px-1.5 py-0.5 bg-scarlet-fire-50 text-scarlet-fire-600 rounded text-xs font-bold">1 Pending</span>
        </div>
        <div className="space-y-2">
          <div className="flex items-center justify-between p-2 bg-glaucous-50/50 rounded border border-glaucous-100">
            <div>
              ...
              <div className="text-xs text-glaucous-600">Grade 12 Batch A</div>
            </div>
            <span className="px-2 py-0.5 bg-emerald-50 text-emerald-700 border border-emerald-100 rounded-full text-xs font-bold">Paid</span>
          </div>
          <div className="flex items-center justify-between p-2 bg-glaucous-50/50 rounded border border-glaucous-100">
            <div>
              <div className="font-bold text-vivid-royal-950">Kabir Sharma</div>
              <div className="text-xs text-glaucous-600">Grade 11 Batch B</div>
            </div>
            {isSent ? (
              <m.span
                initial={{ scale: 0.8, opacity: 0 }}
                animate={{ scale: 1, opacity: 1 }}
                className="px-2 py-0.5 bg-amber-50 text-amber-700 border border-amber-100 rounded-full text-xs font-bold"
              >
                WhatsApp Sent
              </m.span>
            ) : (
              <button
                ...
                className="px-2.5 py-1 bg-coffee-bean-500 hover:bg-coffee-bean-600 text-coffee-bean-950 font-bold rounded text-xs transition-colors cursor-pointer"
              >
                Remind on WhatsApp
              </button>
            )}
        ...
        {isSent && (
          <m.div
            ...
            className="p-2.5 bg-[#d9fdd3] text-neutral-800 rounded-lg rounded-tr-none border border-[#c2f3ba] text-sm leading-snug space-y-1"
          >
            <div className="font-bold text-xs text-emerald-850 flex justify-between">
  ...
  const renderHealthcareWidget = () => {
    ...
    return (
      <div className="bg-white border border-glaucous-200 rounded-xl p-4 shadow-sm space-y-3 font-sans text-sm">
        <div className="flex items-center justify-between border-b border-glaucous-100 pb-2">
          <span className="font-mono text-xs font-bold text-glaucous-600">CALENDAR AUTOMATION</span>
          <span className="px-1.5 py-0.5 bg-vivid-royal-50 text-vivid-royal-600 rounded text-xs font-bold">Today</span>
        </div>
        <div className="space-y-2">
          <div className="flex items-center justify-between p-2 bg-glaucous-50/50 rounded border border-glaucous-100">
            <div>
              <div className="font-bold text-vivid-royal-950">Dr. Mehta - 11:30 AM Slot</div>
              <div className="text-xs text-glaucous-600">Patient: Rahul Verma</div>
            </div>
            <span className={`px-2 py-0.5 rounded-full text-xs font-bold border transition-colors duration-350 ${
              isConfirmed
        ...
        {!isConfirmed ? (
          <div className="p-3 bg-neutral-50 border border-neutral-100 rounded-lg space-y-2">
            <div className="font-semibold text-neutral-500 text-xs uppercase">Automated Patient Alert</div>
            <p className="text-sm text-neutral-800 leading-snug">Hi Rahul! Please confirm your appointment with Dr. Mehta today at 11:30 AM.</p>
            <button
              onClick={() => setInteractiveState({ ...interactiveState, healthcare: "confirmed" })}
              className="w-full py-1.5 bg-vivid-royal-600 hover:bg-vivid-royal-700 text-white font-bold rounded text-xs transition-colors cursor-pointer"
            >
          ...
          <m.div
            initial={{ opacity: 0, y: 10 }}
            animate={{ opacity: 1, y: 0 }}
            className="p-2.5 bg-emerald-50 border border-emerald-100 text-emerald-800 rounded-lg text-sm leading-snug"
          >
            <div className="font-bold text-xs mb-1">✓ APPOINTMENT CONFIRMED</div>
  ...
  const renderSalonWidget = () => {
    ...
    return (
      <div className="bg-white border border-glaucous-200 rounded-xl p-4 shadow-sm space-y-3 font-sans text-sm">
        <div className="flex items-center justify-between border-b border-glaucous-100 pb-2">
          <span className="font-mono text-xs font-bold text-glaucous-600">RECALL TIMELINE</span>
          <span className="text-xs text-glaucous-500 font-bold">28-Day Cycle</span>
        </div>
        <div className="space-y-3 relative before:absolute before:top-2 before:bottom-2 before:left-[11px] before:w-[1px] before:bg-glaucous-200">
          <div className="flex gap-3 items-start relative pl-6">
            ...
            <div>
              ...
              <p className="text-xs text-glaucous-600">Client: Ritu Sen</p>
            </div>
          </div>
          <div className="flex gap-3 items-start relative pl-6">
            <div className="absolute left-[6px] top-1 w-2.5 h-2.5 rounded-full bg-emerald-500 border border-white" />
            <div>
              ...
              <p className="text-xs text-glaucous-600">Sent. Received 5★ rating.</p>
            </div>
          </div>
          <div className="flex gap-3 items-start relative pl-6">
            ...
            <div className="w-full">
              ...
              <p className="text-xs text-glaucous-600 mb-2">Automated rebooking offer.</p>
              {!isSent ? (
                <button
                ...
                <m.div
                  ...
                  className="p-2 bg-[#d9fdd3] text-neutral-800 rounded-lg rounded-tr-none border border-[#c2f3ba] text-xs"
                >
  ...
  const renderGymWidget = () => {
    ...
    return (
      <div className="bg-white border border-glaucous-200 rounded-xl p-4 shadow-sm space-y-3 font-sans text-sm">
        <div className="flex items-center justify-between border-b border-glaucous-100 pb-2">
          <span className="font-mono text-xs font-bold text-glaucous-600">CHECK-IN VALIDATOR</span>
          <span className="px-1.5 py-0.5 bg-scarlet-fire-50 text-scarlet-fire-600 rounded text-xs font-bold">FRONT DESK</span>
        </div>
        <div className="p-3 rounded-lg border flex items-center justify-between gap-4 transition-colors duration-305 bg-neutral-50">
          <div>
            <div className="font-bold text-vivid-royal-950 text-base">Member: Rohan Malhotra</div>
            <div className="text-xs text-neutral-500 mt-0.5">Membership ID: #4092</div>
          </div>
          <span className={`px-2 py-1 rounded text-xs font-bold border transition-all duration-300 ${
            isActive
              ? "bg-emerald-50 text-emerald-700 border-emerald-100"
        ...
        {!isActive ? (
          <div className="p-2.5 bg-red-50/50 border border-red-100 rounded-lg space-y-2">
            <p className="text-xs text-red-900 leading-snug">Membership expired 5 days ago. Click below to simulate WhatsApp renewal link payment.</p>
            <button
              onClick={() => setInteractiveState({ ...interactiveState, gym: "active" })}
              className="w-full py-1.5 bg-coffee-bean-500 hover:bg-coffee-bean-600 text-coffee-bean-950 font-bold rounded text-xs transition-colors cursor-pointer"
            >
          ...
          <m.div
            ...
            className="p-2.5 bg-emerald-50 border border-emerald-100 text-emerald-800 rounded-lg text-sm leading-snug"
          >
            <div className="font-bold text-xs mb-1">✓ PAYMENT RECONCILED</div>
  ...
  const renderProfessionalWidget = () => {
    ...
    return (
      <div className="bg-white border border-glaucous-200 rounded-xl p-4 shadow-sm space-y-3 font-sans text-sm">
        <div className="flex items-center justify-between border-b border-glaucous-100 pb-2">
          <span className="font-mono text-xs font-bold text-glaucous-600">MILESTONE TRACKER</span>
          <span className="text-xs text-glaucous-500 font-bold">Agency Flow</span>
        </div>
        <div className="space-y-2">
          ...
          <div className={`flex items-center justify-between p-2 rounded border transition-colors duration-300 ${
            ...
            <span className="text-xs font-bold">{isApproved ? "Approved" : "Awaiting Signoff"}</span>
          </div>
        </div>

        {!isApproved ? (
          <div className="p-2.5 bg-neutral-50 border border-neutral-100 rounded-lg space-y-2">
            <p className="text-xs text-neutral-800 leading-snug">Client gets a WhatsApp message to tap and approve the milestone.</p>
            <button
          ...
          <m.div
            initial={{ opacity: 0, y: 10 }}
            animate={{ opacity: 1, y: 0 }}
            className="p-2.5 bg-emerald-50 border border-emerald-100 text-emerald-800 rounded-lg text-sm leading-snug space-y-1.5"
          >
            <div className="font-bold text-xs text-emerald-900">✓ MILESTONE SIGNED OFF</div>
  ...
  return (
    <section className="w-full bg-white border-t border-glaucous-200/80 py-20 px-6" id="industries">
      <div className="max-w-7xl mx-auto text-center mb-16">
        ...
        <p className="text-glaucous-800 max-w-xl mx-auto leading-relaxed">
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 bg-ghost-white-50 border border-glaucous-200 rounded-2xl p-8 sm:p-12 items-center">
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="bg-glaucous-100/50 border border-glaucous-200/80 p-5 rounded-2xl">
            <span className="font-mono text-xs text-glaucous-600 uppercase tracking-widest font-bold block mb-3">
          ...
          <div className="bg-glaucous-50 border border-glaucous-200 p-6 rounded-2xl shadow-inner">
            <span className="font-mono text-xs text-coffee-bean-600 uppercase tracking-widest font-bold block mb-4">
              What ADX Builds
            </span>
            <h4 className="text-base font-bold text-vivid-royal-950 mb-4">{industryData[activeInd].solTitle}</h4>
            <div className="flex flex-col gap-4">
              <div className="border-l-2 border-scarlet-fire-400 pl-4">
                <span className="block font-mono text-xs text-glaucous-600 uppercase mb-1">Before ADX</span>
              ...
              <div className="border-l-2 border-coffee-bean-600 pl-4">
                <span className="block font-mono text-xs text-coffee-bean-600 uppercase mb-1">With ADX</span>
