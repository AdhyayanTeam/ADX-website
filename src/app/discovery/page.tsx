      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid lg:grid-cols-[1.1fr_0.9fr] gap-16 items-center">
          <div className="flex flex-col gap-10">
            {steps.map((item) => (
              <div key={item.num} className="border-l-2 border-glaucous-200 hover:border-coffee-bean-500 pl-6 transition-all duration-300">
                <span className="font-mono text-xs text-glaucous-600 tracking-wider block mb-1">
                  STAGE {item.num}
                </span>
                <h3 className="text-xl font-bold text-vivid-royal-950 mb-2">{item.title}</h3>
                <p className="text-sm text-glaucous-800 leading-relaxed">{item.desc}</p>
              </div>
            ))}
          </div>
          ...
          <div className="bg-ghost-white-50 border border-glaucous-200 rounded-lg overflow-hidden shadow-xl">
            ...
            <div className="p-8">
              <h3 className="text-lg font-bold text-vivid-royal-950 mb-4">Operational Status Audit</h3>
              
              <div className="grid grid-cols-2 gap-4 mb-6">
                <div className="bg-glaucous-50 border border-glaucous-200 p-4 rounded">
                  <span className="font-mono text-[9px] text-glaucous-600 uppercase block mb-1">Response Latency</span>
                  <span className="text-lg font-bold text-scarlet-fire-600">12 mins (Avg)</span>
                </div>
                <div className="bg-glaucous-50 border border-glaucous-200 p-4 rounded">
                  <span className="font-mono text-[9px] text-glaucous-600 uppercase block mb-1">Unmatched Leads</span>
                  <span className="text-lg font-bold text-scarlet-fire-600">32% leakage</span>
                </div>
              </div>

              <div className="bg-glaucous-50 border border-glaucous-200 p-4 rounded font-mono text-xs text-glaucous-800 mb-6">
                <div className="text-glaucous-600 font-semibold mb-2 uppercase text-[9px] tracking-widest">Target Solution Spec</div>
                <div className="flex gap-2 items-center justify-center py-2 border-b border-glaucous-200/55">
                  <span className="bg-glaucous-100 px-2 py-0.5 rounded text-[10px]">Lead Intake</span>
                  <span className="text-glaucous-600">→</span>
                  <span className="bg-coffee-bean-50 border border-coffee-bean-200 text-coffee-bean-600 px-2 py-0.5 rounded text-[10px]">Auto-Reply API</span>
                  <span className="text-glaucous-600">→</span>
                  <span className="bg-glaucous-100 px-2 py-0.5 rounded text-[10px]">Tally DB</span>
                </div>
              </div>

              <p className="text-xs text-glaucous-800 leading-relaxed mb-6">
              ...
              <Link
                href="/book"
                className="w-full text-center py-3.5 rounded text-xs font-semibold uppercase tracking-wider text-ghost-white-50 bg-coffee-bean-600 hover:bg-coffee-bean-700 transition-all duration-300 block shadow-[0_0_15px_rgba(237,18,109,0.15)]"
              >
